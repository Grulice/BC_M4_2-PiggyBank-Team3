let getBestProduct = (bankProducts, userInput) => {
    ...
    } else { return "Sorry we can't find appropriate product" }
}

let filterProducts = (bankProducts, userInput) => {
    let filteredProducts = bankProducts.filter((product) => {
        return product.minSumm <= userInput.initSum && product.maxSumm >= userInput.initSum;
    }).filter((product) => {
        return product.minTerm <= userInput.term && product.maxTerm >= userInput.term;
    });
...
let sortResults = (result) => {
    let sortedResult = result.sort((a, b) => { return b.totalSum - a.totalSum });
    let highestSum = sortedResult[0].totalSum;
    let finalResult = sortedResult.filter(option => option.totalSum == highestSum);
    if (finalResult.length > 1) {
        let sortedFinalResult = finalResult.sort((a, b) => { return b.requiredMonthes - a.requiredMonthes });
        let shortestTerm = sortedFinalResult[0].requiredMonthes;
        let superFinalResult = sortedFinalResult.filter(option => option.requiredMonthes == shortestTerm);
